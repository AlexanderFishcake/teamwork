<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ include file="/WEB-INF/views/common/header.jsp" %>
<script>
	$(function(){
		/**
		* 유효성검사
		*/
		document.memberEnrollFrm.onsubmit = function(){
 			var $memberId_ = $(memberId_);
			var $password_ = $(password_);
			var $password2 = $(password2);
			var $memberName = $(memberName);
			//var $birthday = $(birthday);
			//var $email = $(email);
			var $phone = $(phone);
			//var $address = $(address);
			var $hobby = $("[name=hobby]:checked");
			
			if(/^.{4,}$/.test($memberId_.val()) == false){
				alert("유효한 아이디를 입력하세요.");
				$memberId_.select();
				return false;
			}
			if(/^.{4,}$/.test($password_.val()) == false){
				alert("유효한 비밀번호를 입력하세요.");
				$password_.select();
				return false;
			} 

			if($password2.val()!=$password_.val()){
				alert("패스워드 확인을 다시 확인하십시오");
				$password2.select();
				return false;
			} 
						
			if(/^[가-힣a-zA-Z]+/.test($memberName.val()) == false){
				alert("유효한 이름을 입력하세요.");
				$memberName.select();
				return false;
			} 
			if(/^\d{10,11}$/.test($phone.val()) == false){
				alert("전화번호를 다시 확인해주세요.");
				$password_.select();
				return false;
			} 
			if($hobby.val()==undefined) {
				alert("취미를 한 개 이상 선택해주세요.");
				$password_.select();
				return false;
			} 
		}
/* 		$("[name=memberEnrollFrm]").submit(function(){

		}); */
	});
</script>
<section id=enroll-container>
	<h2>회원 가입 정보 입력</h2>
	<form name="memberEnrollFrm" action="" method="post">
		<table>
			<tr>
				<th>아이디<sup>*</sup></th>
				<td>
					<input type="text" placeholder="4글자이상" name="memberId" id="memberId_" required>
				</td>
			</tr>
			<tr>
				<th>패스워드<sup>*</sup></th>
				<td>
					<input type="password" name="password" id="password_" required><br>
				</td>
			</tr>
			<tr>
				<th>패스워드확인<sup>*</sup></th>
				<td>	
					<input type="password" id="password2" required><br>
				</td>
			</tr>  
			<tr>
				<th>이름<sup>*</sup></th>
				<td>	
				<input type="text"  name="memberName" id="memberName" required><br>
				</td>
			</tr>
			<tr>
				<th>생년월일</th>
				<td>	
				<input type="date" name="birthday" id="birthday" ><br />
				</td>
			</tr> 
			<tr>
				<th>이메일</th>
				<td>	
					<input type="email" placeholder="abc@xyz.com" name="email" id="email"><br>
				</td>
			</tr>
			<tr>
				<th>휴대폰<sup>*</sup></th>
				<td>	
					<input type="tel" placeholder="(-없이)01012345678" name="phone" id="phone" maxlength="11" required><br>
				</td>
			</tr>
			<tr>
				<th>주소</th>
				<td>	
					<input type="text" placeholder="" name="address" id="address"><br>
				</td>
			</tr>
			<tr>
				<th>성별 </th>
				<td>
					<input type="radio" name="gender" id="gender0" value="M" checked>
					<label for="gender0">남</label>
					<input type="radio" name="gender" id="gender1" value="F">
					<label for="gender1">여</label>
				</td>
			</tr>
			<tr>
				<th>취미 </th>
				<td>
					<input type="checkbox" name="hobby" id="hobby0" value="운동"><label for="hobby0">운동</label>
					<input type="checkbox" name="hobby" id="hobby1" value="등산"><label for="hobby1">등산</label>
					<input type="checkbox" name="hobby" id="hobby2" value="독서"><label for="hobby2">독서</label><br />
					<input type="checkbox" name="hobby" id="hobby3" value="게임"><label for="hobby3">게임</label>
					<input type="checkbox" name="hobby" id="hobby4" value="여행"><label for="hobby4">여행</label><br />
				</td>
			</tr>
		</table>
		<input type="submit" value="가입" >
		<input type="reset" value="취소">
	</form>
</section>
<%@ include file="/WEB-INF/views/common/footer.jsp" %>
