...
/**
 * 서블릿 동록을 자동으로 처리해주는 annotation @WebServlet
 * 
 * - urlPatterns
 * - name:String 
 */
...
public class MemberLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//	비밀번호가 틀릴때 member != null !password.equals(member.getPassword())
...
//			request.setAttribute("msg", "로그인에 성공했습니다.");
			...
			//로그인한 사용자 정보
			//request.getSession(create:booloean) : 새로 생성여부 기본값 true
			
//			System.out.println(session.getId());
			session.setAttribute("loginMember", member);
			
			//session timeout : web.xml보다 우선순위 높음. 초단위로 작성.
//			session.setMaxInactiveInterval(30);
			...
			if(saveId!=null) {
				//saveId 체크시
				c.setMaxAge(60*60*24*7);	//7일짜리 영속쿠기로 지정
			}
			else {
				//saveId 체크해제시
				c.setMaxAge(0);	//0으로 지정해서 즉시 삭제, 음수로 지정하면 session 종료시 제거
			}
